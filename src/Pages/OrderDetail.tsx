...
export default function OrderDetailPage() {
  ...
  // Loading and Error states with high top padding for mobile visibility
  ...
  return (
    <div style={{ 
      maxWidth: 1200, 
      // Mobile pe 100px aur Desktop pe 80px margin taaki header ke peeche na chhup jaye
      ...
      {/* Back Button - Ab ye hamesha upar dikhega */}
      ...
      <div style={{ 
          ...
          <div style={{ flex: "1 1 200px" }}>
              <h1 style={{ margin: 0, fontSize: isMobile ? 26 : 38, fontWeight: 900 }}>Order #{data.id}</h1>
          ...
          <div style={{ textAlign: isMobile ? "left" : "right" }}>
              <div style={{ fontWeight: 900, fontSize: 28, color: "#111827" }}>{formatMoney(data.total_amount)}</div>
              <div style={{ marginTop: 4, color: "#6b7280", fontWeight: 700 }}>Total Items: {totalQty}</div>
          </div>
      </div>

      {/* Content Grid */}
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "0.9fr 1.1fr", gap: 18, alignItems: "start" }}>
        
        {/* Tracking Section */}
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 20, background: "#fff" }}>
          <div style={{ fontWeight: 900, fontSize: 22, marginBottom: 20 }}>Order Status</div>
        ...
        <div style={{ display: "grid", gap: 18 }}>
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 20, background: "#fff" }}>
            <div style={{ fontWeight: 900, fontSize: 20, marginBottom: 12 }}>Shipping Address</div>
            <div style={{ color: "#374151", lineHeight: 1.7, fontSize: 15 }}>
              <b style={{ fontSize: 18, display: "block", marginBottom: 4 }}>{data.shipping_full_name}</b>
              {data.shipping_phone}<br />
              {data.shipping_line1} {data.shipping_line2}<br />
              {data.shipping_city}, {data.shipping_state} - {data.shipping_postal_code}<br />
              {data.shipping_country}
            </div>
          </div>

          <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 20, background: "#fff" }}>
            <div style={{ fontWeight: 900, fontSize: 20, marginBottom: 12 }}>Payment Details</div>
            <div style={{ display: "grid", gap: 10 }}>
              <div style={detailRow}>
                <span style={{ color: "#6b7280" }}>Order ID</span>
                <span style={{ fontWeight: 600, fontSize: 13 }}>{data.razorpay_order_id || "-"}</span>
              </div>
              <div style={detailRow}>
                <span style={{ color: "#6b7280" }}>Payment ID</span>
                <span style={{ fontWeight: 600, fontSize: 13 }}>{data.razorpay_payment_id || "-"}</span>
              </div>
              <div style={{ ...detailRow, fontWeight: 900, borderTop: "1px solid #eee", paddingTop: 12, marginTop: 6 }}>
      ...
      {/* Ordered Items List */}
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 20, background: "#fff" }}>
        <div style={{ fontWeight: 900, fontSize: 22, marginBottom: 16 }}>Items in this Order</div>
        <div style={{ display: "grid", gap: 14 }}>
            {data.items?.map((it) => (
                <div key={it.id} style={{ 
                    display: "grid", 
                    ...
                    <img 
                      src={it.product_image || "https://via.placeholder.com/110"} 
                      alt={it.product_title}
