...
export default function CartPage() {
  ...
  // Temporary quantity map for editing
  ...
  // Reset temporary quantities whenever the items change (e.g., after load)
  useEffect(() => {
    const initialMap: Record<string, string> = {};
    items.forEach((item) => {
      initialMap[`${item.mode}-${item.id}`] = String(item.quantity);
    });
    setTempQtyMap(initialMap);
  }, [items]);
  ...
  function handleQtyInputChange(item: HybridCartItem, rawValue: string) {
    const key = `${item.mode}-${item.id}`;
    setTempQtyMap((prev) => ({ ...prev, [key]: rawValue }));
  }

  async function handleQtyBlur(item: HybridCartItem) {
    const key = `${item.mode}-${item.id}`;
    const rawValue = tempQtyMap[key] || String(item.quantity);
    let newQty = parseInt(rawValue, 10);
    if (isNaN(newQty) || newQty < 1) newQty = 1;
    if (newQty !== item.quantity) {
      await changeQty(item, newQty);
    } else {
      // Ensure the temporary value matches the actual quantity (e.g., if user typed and then reverted)
      setTempQtyMap((prev) => ({ ...prev, [key]: String(item.quantity) }));
    }
  }
      ...
      >
        <div style={{ display: "grid", gap: 14 }}>
          {items.map((it) => {
            const unitPrice = Number(it.unit_price || 0);
            const lineTotal = Number(it.line_total || unitPrice * it.quantity);
            const key = `${it.mode}-${it.id}`;
            const tempQty = tempQtyMap[key] ?? String(it.quantity);
                      ...
                      >
                        <button
                          onClick={() => {
                            ...
                            if (newQty >= 1) changeQty(it, newQty);
                          }}
                          disabled={busyId === it.id || it.quantity <= 1}
                          style={qtyBtnStyle}
                        >
                        ...
                        <input
                          ...
                          pattern="[0-9]*"
                          value={tempQty}
                          onChange={(e) => handleQtyInputChange(it, e.target.value)}
