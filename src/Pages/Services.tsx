...
type ServicesApiResponse = {
  ...
  why_choose_section_title: string;
...
const defaultData: ServicesApiResponse = {
  hero_title: "Services",
  hero_background_image: "/assets/images/services-hero.jpg",
  services_section_title: "Our Services",
  why_choose_section_title: "Why Choose Tunturu Tool",
  process_section_title: "Our Process",
  cta_title: "Ready to get the job done?",
  cta_description:
    "Contact us today to learn more about how we can assist you with your tools and hardwear.",
...
export default function Services() {
  const [_showTop, setShowTop] = useState(false);
  const [pageData, setPageData] = useState<ServicesApiResponse>(defaultData);
  ...
  useEffect(() => {
    const fetchServicesData = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Failed to fetch services data: ${res.status}`);
        }
        ...
        setPageData({
          ...defaultData,
          ...data,
          services_cards: data?.services_cards?.length ? data.services_cards : defaultData.services_cards,
          why_choose_cards: data?.why_choose_cards?.length ? data.why_choose_cards : defaultData.why_choose_cards,
          process_cards: data?.process_cards?.length ? data.process_cards : defaultData.process_cards,
        });
      } catch (error) {
        console.error("Services API fallback used:", error);
    ...
    <>
      ...
      <div className="tPage">
        ...
        <section className="tSection tSectionGray">
          <div className="tContainer">
            <div className="tTitleBlock">
              <h2 className="tH2">{pageData.services_section_title}</h2>
              <div className="tUnderline" />
            </div>

            <div className="tGrid4">
              {pageData.services_cards.map((c) => (
                <div key={c.title} className="tCard">
                  <div className="tIconWrap">{renderIcon(c.icon)}</div>
                  <h3 className="tCardTitle">{c.title}</h3>
                  <p className="tCardDesc">{c.desc}</p>
                </div>
              ))}
        ...
        <section className="tSection tSectionGray">
          <div className="tContainer">
            <div className="tTitleBlock">
              <h2 className="tH2">{pageData.why_choose_section_title}</h2>
              <div className="tUnderline wide" />
            </div>

            <div className="tGrid3">
              {pageData.why_choose_cards.map((c) => (
                <div key={c.title} className="tCard">
                  <div className="tIconWrap">{renderIcon(c.icon)}</div>
                  <h3 className="tCardTitle">{c.title}</h3>
                  <p className="tCardDesc">{c.desc}</p>
        ...
        <section className="tSection tSectionGray">
          <div className="tContainer">
            <div className="tTitleBlock">
              <h2 className="tH2">{pageData.process_section_title}</h2>
              <div className="tUnderline" />
            </div>

            <div className="tGrid4">
              {pageData.process_cards.map((c) => (
                <div key={c.title} className="tCard">
                  <div className="tIconWrap">{renderIcon(c.icon)}</div>
                  <h3 className="tCardTitle">{c.title}</h3>
                  <p className="tCardDesc">{c.desc}</p>
        ...
        <section className="tCTA">
          <div className="tContainer tCTACenter">
            <h3 className="tCTATitle">{pageData.cta_title}</h3>
            <p className="tCTASub">{pageData.cta_description}</p>
            <a className="tCTAButton" href={pageData.cta_button_link}>
...
function renderIcon(iconName: string) {
  switch (iconName) {
    case "hammer_wrench":
      return <IconHammerWrench />;
    case "tools":
      return <IconTools />;
    case "truck_cart":
      return <IconTruckCart />;
    case "building_people":
      return <IconBuildingPeople />;
    case "calendar_check":
      return <IconCalendarCheck />;
    case "headset":
      return <IconHeadset />;
    case "receipt_check":
      return <IconReceiptCheck />;
    case "docs_shield":
      return <IconDocsShield />;
    case "monitor_gear":
      return <IconMonitorGear />;
    case "hand_gear_check":
      return <IconHandGearCheck />;
    case "support_24":
      return <IconSupport24 />;
    default:
      return <IconHammerWrench />;
  }
}
