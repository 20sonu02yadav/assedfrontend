...
type PrivacyApiResponse = {
  ...
  data_security_heading: string;
  data_security_text: string;
  changes_heading: string;
  changes_text: string;
  sections: {
    personal_info: PrivacySectionData;
    auto_info: PrivacySectionData;
    use_info: PrivacySectionData;
    sharing_info: PrivacySectionData;
    cookies: PrivacySectionData;
    rights: PrivacySectionData;
  };
};
...
const defaultData: PrivacyApiResponse = {
  ...
  hero_background_image:
    "https://dev-tunturu.pantheonsite.io/wp-content/uploads/2026/02/pexels-mikhail-nilov-7735625.jpg",
  intro_heading: "Introduction",
  intro_text:
    "At Tunturu Tools, we respect your privacy and are committed to protecting your personal information. This Privacy Policy explains how we collect, use, and safeguard your data when you visit our website or make a purchase.",
  contact_email: "support@tunturu.co.in",
  data_security_heading: "Data Security",
  data_security_text:
    "We implement industry-standard security measures including SSL encryption and secure servers to protect your data.",
  ...
  sections: {
    ...
    use_info: {
      ...
      intro_text: "We use your information to:",
      outro_text: "",
      items: [
        "Process and fulfill orders",
        "Provide customer support",
        "Improve our website and services",
        "Send order updates and promotional emails",
        "Prevent fraud and unauthorized activity",
      ],
    },
    sharing_info: {
      heading: "Sharing of Information",
      intro_text: "We may share data with:",
      outro_text: "",
      items: [
        "Payment gateways",
        "Shipping partners",
    ...
    cookies: {
      heading: "Cookies",
      intro_text: "We use cookies to:",
      outro_text: "You can disable cookies in your browser settings.",
      items: [
        "Improve user experience",
        "Analyze website traffic",
        "Remember cart information",
      ],
    },
    rights: {
      heading: "Your Rights",
      intro_text: "You have the right to:",
      outro_text: "",
      items: [
...
export default function Privacy() {
  const [_showTop, setShowTop] = useState(false);
  const [pageData, setPageData] = useState<PrivacyApiResponse>(defaultData);
  ...
  useEffect(() => {
    const fetchPrivacyData = async () => {
      try {
        const res = await fetch(PRIVACY_API);
        if (!res.ok) {
          throw new Error(`Failed to fetch privacy data: ${res.status}`);
        }
        ...
        console.error("Privacy API fallback used:", error);
    ...
    <>
      ...
      <div className="pPage">
        ...
        >
          <div className="pHeroOverlay" />
          <div className="pHeroInner">
            <h1 className="pHeroTitle">{pageData.hero_title}</h1>
          </div>
        </section>

        <section className="pContentWrap">
          <div className="pContainer pContent">
            <h2 className="pH2">{pageData.intro_heading}</h2>
            <p className="pPara">{pageData.intro_text}</p>
            ...
            <div className="pTwoCols">
              <div className="pCol">
                <h3 className="pH3">{pageData.sections.personal_info.heading}</h3>
                <ul className="pList">
                  {pageData.sections.personal_info.items.map((item, index) => (
              ...
              <div className="pCol">
                <h3 className="pH3">{pageData.sections.auto_info.heading}</h3>
                <ul className="pList">
                  {pageData.sections.auto_info.items.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="pTwoCols">
              <div className="pCol">
                <h2 className="pH2 small">{pageData.sections.use_info.heading}</h2>
                {pageData.sections.use_info.intro_text ? (
                  <p className="pPara">{pageData.sections.use_info.intro_text}</p>
                ) : null}
                <ul className="pList">
                  {pageData.sections.use_info.items.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
              ...
              <div className="pCol">
                <h2 className="pH2 small">{pageData.sections.sharing_info.heading}</h2>
                {pageData.sections.sharing_info.intro_text ? (
                  <p className="pPara">{pageData.sections.sharing_info.intro_text}</p>
                ) : null}
                <ul className="pList">
                  {pageData.sections.sharing_info.items.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="pTwoCols">
              <div className="pCol">
                <h2 className="pH2 small">{pageData.sections.cookies.heading}</h2>
                {pageData.sections.cookies.intro_text ? (
                  <p className="pPara">{pageData.sections.cookies.intro_text}</p>
                ) : null}
                <ul className="pList">
                  {pageData.sections.cookies.items.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
                {pageData.sections.cookies.outro_text ? (
                  <p className="pPara muted">{pageData.sections.cookies.outro_text}</p>
                ) : null}
              </div>

              <div className="pCol">
                <h2 className="pH2 small">{pageData.sections.rights.heading}</h2>
                {pageData.sections.rights.intro_text ? (
                  <p className="pPara">{pageData.sections.rights.intro_text}</p>
                ) : null}
                <ul className="pList">
                  {pageData.sections.rights.items.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>

                <p className="pPara">
                  Contact us at: <span className="pInline">{pageData.contact_email}</span>
                </p>
              </div>
            </div>
            ...
            <h2 className="pH2">{pageData.data_security_heading}</h2>
            <p className="pPara">{pageData.data_security_text}</p>

            <div className="pSpacer" />

            <h2 className="pH2">{pageData.changes_heading}</h2>
            <p className="pPara">{pageData.changes_text}</p>
          </div>
        </section>
      </div>
    </>
